...
export async function criarPedidoVenda(pedido: PedidoVenda & { idEmpresa: number }): Promise<any> {
  try {
    ...
    // Determinar o modelo da nota
    let modeloNota = 974; // Padrão

    if ((pedido as any).MODELO_NOTA && (pedido as any).MODELO_NOTA !== '') {
      modeloNota = parseInt((pedido as any).MODELO_NOTA);
    } else if (pedido.CODTIPOPER) {
      modeloNota = parseInt(pedido.CODTIPOPER);
    }

    console.log(`📋 Modelo da nota definido: ${modeloNota}`);
    ...
    console.log(`📅 Data validada: ${dataFinal} (original: ${dataNegociacao})`)
